...
const DEADLINE_SEC    = parseInt(process.env.DEADLINE_MINUTES, 10) * 60; // e.g. 10 → 600s
const GAS_LIMIT       = parseInt(process.env.GAS_LIMIT, 10);          // e.g. 210000
...
/**
 * Swap function:
 * - from: token-in address (WNATIVE for native→token)
 * - to:   token-out address
 * - amountInRaw: human-readable amount string (e.g. "0.001")
 */
async function swapToken(from, to, amountInRaw) {
  ...
  const rawOutMin  = amountsOut[amountsOut.length - 1]
    .mul(Math.floor((1 - SLIPPAGE) * 1000))
    .div(1000);

  console.log(`\n🔄 Swapping ${amountInRaw} (decimals ${decimalsIn}) via [${path.join(' → ')}]`);
  console.log(`    estimated out: ${ethers.utils.formatUnits(amountsOut.pop(), decimalsIn)}`);
  console.log(`    minimum out (slippage ${(SLIPPAGE*100).toFixed(2)}%): ${ethers.utils.formatUnits(rawOutMin, decimalsIn)}`);
  ...
  const got      = afterBal.sub(beforeBal);
  console.log(`✅ Received: ${ethers.utils.formatUnits(got, decimalsIn)} tokens`);
}

// === Example usage: define your pairs here ===
const SWAP_PAIRS = [
...
(async () => {
  ...
  for (const { from, to } of SWAP_PAIRS) {
    try {
      ...
      console.error('❌ Error swapping', from, '→', to, e);
    }
  }
  console.log('\n🎉 All done!');
})();
